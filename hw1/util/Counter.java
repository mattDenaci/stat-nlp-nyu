...
public class Counter<E> implements Serializable {
	...
	public void discount(double discount) {
		List<E> keysToRemove = new ArrayList<E>();
		for (E key : keySet()) {
			if (getCount(key) - discount > 0) {
				setCount(key, getCount(key) - discount);
			}else{
				keysToRemove.add(key);
			}
		}
		for(E key: keysToRemove){
			entries.remove(key);
		}
		...
		for (E key : keySet()) {
			if (getCount(key) - discount > 0) {
				setCount(key, Math.max(getCount(key) - discount, 1));
			}
		}
		*/
